	...
	*/
	function applyCoefficients(marks, overrides) {
		for (const mod of marks) for (const sub of mod.subjects) for (const mark of sub.marks) {
			if (overrides && mark._code && overrides.has(mark._code)) mark.coefficient = overrides.get(mark._code);
		...
		for (const mod of marks) {
			...
			for (const sub of mod.subjects) {
				...
				if (subWeight > 0) for (const mark of sub.marks) mark.coefficient = mark.coefficient / subWeight;
				if (sub.average != null) {
	...
	*/
	...
	function renderSubject(subject, moduleId) {
		...
		const marksContent = subject.marks.map((mark) => {
			...
			}, formatGrade(mark.value)), "\xA0/ 20")), ...meta.length ? [h("div", { class: "class-average" }, h("span", { class: "parenthesis" }, "("), meta.join(", "), h("span", { class: "parenthesis" }, ")"))] : []);
		});
